...
description: Scaffold a new git-likeness command for mcagit end to end - research real-git semantics, wire the CLI, implement in Repo/, add tier tests, and update the README. Use when adding a git work-alike subcommand (e.g. "add mcagit worktree", "implement notes", "next git-likeness tier").
---
...
5. **Docs**: README command list (the `mcagit <cmd>` block in Version control), the Tests paragraph, and Limitations if the implementation is partial (e.g. "non-interactive only"). The README's git-likeness framing is per-tier — match the existing commit message style: `Tier N git-likeness: <features>`.
